...
class Doctrine extends \lithium\console\Command
{
    ...
    public function run($args = array())
    {
        ...
        $modelPath = LITHIUM_APP_PATH . '/models';

        if (Environment::is('test')) {
            $modelPath = LITHIUM_LIBRARY_PATH . '/sagalaya/tests/mocks/models';
        }
        ...
        $defaults = array(
            ...
            'useModelDriver' => true,
            'mapping' => array('class' => null, 'path' => $modelPath),
            'configuration' => null,
            'eventManager' => null
        );
        ...
        foreach ($args as &$arg) {
            ...
            if (strstr($arg, '--env')) {
                Environment::set(substr($arg, strpos('=', $arg) + 1));
                unset($args[$i]);
            }
            $i++;
        }
        ...
        //Annotation Driver
        $driver = $config->newDefaultAnnotationDriver(array($modelPath));
